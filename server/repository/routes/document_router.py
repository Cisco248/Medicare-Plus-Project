...
@document_router.get(
    "/documents/{document_id}/file", status_code=200, tags=["Documents"]
)
...
def delete_document(
    ...
    document = _get_owned_document(document_id, auth["uid"], db)
    DocumentMiddleware.delete_file(str(document.file_path))
    db.delete(document)
    db.commit()
